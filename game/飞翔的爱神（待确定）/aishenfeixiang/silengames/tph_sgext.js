...
function sg_clay_ad(adid) {
	try {
		if (window.adscreen != undefined) {
			adscreen.setAttribute('style','display: block');
			if (window.adcont == undefined) {
				adcont = document.createElement("iframe");
				adcont.srcdoc = '<script language="javascript" src="http://www.cpmfun.com/getad.php?'+adid+'"></script><script language="javascript" src="http://www.cpmfun.com/getad.php?'+adid+'"></script>';
				adcont.id = "sg-clay-cont";
				adcont.scrolling = "no";
				adscreen.appendChild(adcont);
			}
		}
...
sg_clay_init_try = 5;
function sg_clay_init(scoreid) {
	try {
		if (Clay.isReady) {
			ClayScore = new Clay.Leaderboard( { id: scoreid, filter: ['day', 'month', 'year'] } );
		} else {
			if (sg_clay_init_try > 0) {
				sg_clay_init_try-=1;
				setTimeout( function(){
					 sg_clay_init(scoreid);
				},2000)
			}
		}
	} catch(e) {};
